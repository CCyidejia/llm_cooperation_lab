...
class PublicGoodsAgent(AgentBase):
    ...
    def _build_history_string(self, all_agent_names: list) -> str:
        ...
        history_lines = []
        history_lines.append("History of previous rounds (global outcomes):")
    ...
    def _build_personal_history_string(self) -> str:
        ...
        for round_summary in self.history:
            r = round_summary["round"]
            # Get personal contribution from round summary
    ...
    async def choose_action(self, initial_endowment: int, public_pool_multiplier: float, num_agents: int, total_rounds: int, memory_size: int = 5):
        ...
        personal_history_text = self._build_personal_history_string()
        ...
        # Add personal history if available
        if personal_history_text:
            final_prompt += f"{personal_history_text}\n\n"
        
        # Add global history
        final_prompt += (
            f"{histories_text}\n\n"
            "***CRITICAL INSTRUCTION***:\n"
            "Based ONLY on the rules and history above (including your own past choices and payoffs),\n"
            "determine your contribution amount for this round.\n\n"
...
async def main():
    ...
    experiment_time = input("Please enter experiment folder name (e.g., 'PG_test1'): ").strip()
