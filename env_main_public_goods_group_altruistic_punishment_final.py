...
class PublicGoodsAgent(AgentBase):
    ...
    def _build_history_string(self, all_agent_names: list) -> str:
        """Return only the participant's own previous-round result.

        Previous information about other players remains confidential. This
        preserves anonymous identities while allowing the participant to learn
        from the consequences of its own previous decision.
        """
        if not self._my_history:
            return (
                "There is no previous-round result because this is the first round. "
                "No previous information about other players is available."
            )

        return (
            "Your previous-round result:\n"
            f"- Your contribution: {self._my_history[-1]} coins\n"
            f"- Your cost of punishment sent: {self._punishment_sent_history[-1]} coins\n"
            f"- Your payoff reduction from punishment received: {self._punishment_received_history[-1]} coins\n"
            f"- Your final payoff: {self._outcome_history[-1]} coins\n"
            "No previous contribution, punishment, payoff, or identity information "
            "about other players is available."
        )
    ...
    async def choose_action(self, initial_endowment: int, public_pool_multiplier: float, num_agents: int, total_rounds: int, memory_size: int = 5):
        """Choose a contribution using only the participant's own prior result."""
        # Expose only the participant's own previous-round outcome.
    ...
    async def choose_punishment_actions(self, round_contributions: dict, punishment_point_range=(0, 10)):
        ...
        own_contribution = round_contributions[self.name]
        contribution_lines = [
        ...
        prompt_context = (
            f"Your own current-round contribution: {own_contribution} coins\n\n"
            "Other players' current-round contributions, each linked to its "
            "temporary anonymous label:\n"
...
if __name__ == "__main__":
    ...
    NUM_EXPERIMENT_RUNS = 3  # Run one experiment
