...
# -*- coding: utf-8 -*-
"""
Trust Game Population + HR 名誉机制。

第一整个群体轮次不提供名誉；从第二个群体轮次开始，Trustor 可选择查看当前
Trustee 的 Summary、Detail、Both 或 None。名誉只使用上一群体轮次结束前的快照，
本轮产生的记录在整轮结束后统一写入。

与 baseline 保持一致，Trustor 和 Trustee 均继续获得当前伙伴的私人双边历史；
从第二个群体轮次开始，Trustor 还可在该历史基础上选择查看公共 HR 名誉信息。

连续金额博弈中的可信回报定义为：Trustor 发送额大于 0，且 Trustee 返还额不低于
Trustor 原始发送额。HR 名誉分为可信回报次数 / 获得回报机会次数。
"""
# This variant preserves baseline private bilateral history and adds public HR
# information on top of it for Trustor decisions.
import os
import json
...
# 连续金额博弈到论文二元“可信回报”的映射：返还至少覆盖 Trustor 原始发送额。
TRUSTWORTHY_RETURN_SENT_RATIO = 1.0
...
class TrustGamePopulationReputationAgent(AgentBase):
    ...
    def __init__(self, id: int, name: str, profile: str):
        ...
        # HR 名誉底层变量（只记录 sent > 0、Trustee 实际获得回报机会的交互）
        self.history_trustee_count = 0
        self.history_trustworthy_count = 0
        self.history_total_received = 0
        self.history_total_returned = 0
        self.history_trustee_records = []
    ...
    async def dump(self) -> dict:
        ...
        try:
            dump_data = {
                ...
                "history_trustworthy_count": self.history_trustworthy_count,
                "history_total_received": self.history_total_received,
                "history_total_returned": self.history_total_returned,
                "history_trustee_records": self.history_trustee_records,
    ...
    async def load(self, dump_data: dict):
        ...
        try:
            ...
            self.history_trustworthy_count = dump_data.get("history_trustworthy_count", getattr(self, "history_trustworthy_count", 0))
            ...
            self.history_trustee_records = dump_data.get("history_trustee_records", getattr(self, "history_trustee_records", []))
    ...
    async def act(self, interaction_num: int, role: str, initial_funds: int,
        ...
        try:
            ...
            # 保留 baseline 的私人双边历史；公共 HR 信息仅在需要时额外追加。
            # This new version intentionally preserves baseline bilateral
            # history; public HR information is appended separately.
            ...
            role_profile = (
                ...
                "Your return decision is publicly visible and is recorded in your HR reputation.\n"
                "Your reputation will affect how much future investment you receive from Trustors, so returning a fair amount protects your future investment opportunities."
            )
    ...
    def _parse_reputation_view_choice(self, content: str) -> str:
        """解析 None/Summary/Detail/Both；无法解析时视为 None。"""
        if not content or not str(content).strip():
            return "none"
        m = re.search(r"\b(none|summary|detail|both)\b", content.strip(), re.IGNORECASE)
        if not m:
            return "none"
        return m.group(1).lower()

    async def choose_reputation_view(self, population_round: int, partner_name: str) -> str:
        """阶段1：选择查看 Trustee 的汇总名誉、详细历史、二者或均不查看。"""
        prompt = (
            f"{self._profile}\n"
            f"Population round {population_round}\n"
            "---\n"
            f"You will next decide how many coins to send to {partner_name} (they will be Trustee).\n"
            "You may view this partner's public history from earlier population rounds. "
            "Choose Summary (aggregate trustworthy-response record), Detail (period-by-period record), "
            "Both, or None. Answer with exactly one word: Summary, Detail, Both, or None.\n"
        )
        role_profile = (
            f"You are {self._name}, Trustor in a game.\n"
            "Goal: Maximize cumulative coins.\n"
        ...
        try:
            ...
            return self._parse_reputation_view_choice(content)
        except Exception as e:
            self.logger.error(f"[{self._name}] choose_reputation_view failed: {e}")
            self._error_count += 1
            return "none"
    ...
    async def act_trustor_investment(
        ...
        try:
            self._current_funds = current_funds
            # Preserve the baseline bilateral history and append public HR as
...
def format_trustee_reputation_addon(
    trustee_name: str,
    snapshot: dict,
    view_choice: str,
) -> str | None:
    """根据轮初快照生成论文式 HR Summary、Detail、Both 或 None。"""
    choice = (view_choice or "none").lower()
    if choice == "none":
        return None

    count = int(snapshot.get("trustee_count", 0))
    trustworthy_count = int(snapshot.get("trustworthy_count", 0))
    records = list(snapshot.get("records", []))

    if count <= 0:
        return (
            f"Public HR reputation of {trustee_name}: not available "
            "(no prior opportunity to respond as Trustee)."
        )

    sections = []
    if choice in {"summary", "both"}:
        reputation = trustworthy_count / count
        sections.append(
            f"[Summary] Public HR reputation of {trustee_name}: "
            f"trustworthy responses / prior Trustee response opportunities = "
            f"{trustworthy_count} / {count} = {reputation:.4f}. "
            f"A response is classified as trustworthy when returned coins are at least "
            f"{TRUSTWORTHY_RETURN_SENT_RATIO:g}× the Trustor's original sent amount."
        )

    if choice in {"detail", "both"}:
        detail_lines = [f"[Detail] Prior Trustee responses of {trustee_name}:"]
        for record in records:
            status = "Trustworthy" if record.get("trustworthy") else "Untrustworthy"
            detail_lines.append(
                f"Population round {record.get('population_round')}: "
                f"sent={record.get('sent_amount')}, received={record.get('received_amount')}, "
                f"returned={record.get('returned_amount')} -> {status}."
            )
        sections.append("\n".join(detail_lines))
...
class TrustGamePopulationReputationEnvironment:
    ...
    def __init__(self, num_agents: int, initial_funds: int,
        ...
        # 每个群体轮次开始时冻结；本轮交互完成后统一提交更新。
        self._reputation_snapshot = {}
        self._pending_reputation_updates = []
        self._active_population_round = None
    ...
    def reset(self):
        ...
        for agent in self.agents:
            ...
            if hasattr(agent, "history_trustee_count"):
                ...
                agent.history_trustworthy_count = 0
                agent.history_total_received = 0
                agent.history_total_returned = 0
                agent.history_trustee_records = []
    ...
    def _start_population_round(self, population_round: int):
        """冻结轮初名誉，确保本轮所有 Trustor 只看到此前轮次的信息。"""
        self._active_population_round = population_round
        self._pending_reputation_updates = []
        self._reputation_snapshot = {
            agent._id: {
                "trustee_count": agent.history_trustee_count,
                "trustworthy_count": agent.history_trustworthy_count,
                "total_received": agent.history_total_received,
                "total_returned": agent.history_total_returned,
                "records": [dict(record) for record in agent.history_trustee_records],
            }
            for agent in self.agents
        }

    def _queue_trustee_reputation_update(
        self,
        population_round: int,
        interaction_num: int,
        trustee: TrustGamePopulationReputationAgent,
        sent_amount: int,
        received_amount: int,
        returned_amount: int,
    ):
        """缓存本轮有实际回报机会的 Trustee 行为；sent=0 时不形成 HR 回应记录。"""
        if sent_amount <= 0:
            return
        trustworthy = returned_amount >= TRUSTWORTHY_RETURN_SENT_RATIO * sent_amount
        self._pending_reputation_updates.append({
            "trustee_id": trustee._id,
            "population_round": population_round,
            "interaction": interaction_num,
            "sent_amount": int(sent_amount),
            "received_amount": int(received_amount),
            "returned_amount": int(returned_amount),
            "trustworthy": bool(trustworthy),
        })

    def _commit_population_round_reputation_updates(self):
        """群体轮次结束后一次性提交本轮 HR 记录。"""
        for update in self._pending_reputation_updates:
            trustee = self._get_agent_by_id(update["trustee_id"])
            if trustee is None:
                continue
            trustee.history_trustee_count += 1
            trustee.history_trustworthy_count += int(update["trustworthy"])
            trustee.history_total_received += update["received_amount"]
            trustee.history_total_returned += update["returned_amount"]
            trustee.history_trustee_records.append({
                key: value
    ...
    async def run_interaction(self, interaction_num: int) -> dict:
        ...
        pairs_per_population_round = self.num_agents // 2
        population_round = (interaction_num - 1) // pairs_per_population_round + 1
        is_population_round_start = (interaction_num - 1) % pairs_per_population_round == 0
        is_population_round_end = interaction_num % pairs_per_population_round == 0

        if is_population_round_start:
            self._start_population_round(population_round)
        ...
        try:
            ...
            # 第一整个群体轮次不提供名誉；从第二群体轮次开始选择查看类型。
            view_reputation: bool | None = None
            reputation_view_choice: str | None = None
            reputation_addon: str | None = None
            trustee_snapshot = self._reputation_snapshot.get(trustee._id, {})
            if population_round >= 2:
                reputation_view_choice = await trustor.choose_reputation_view(
                    population_round, trustor_partner
                )
                view_reputation = reputation_view_choice != "none"
                reputation_addon = format_trustee_reputation_addon(
                    trustee.name,
                    trustee_snapshot,
                    reputation_view_choice,
                )
            ...
            # HR：本轮决策使用的轮初快照名誉，而不是本轮实时更新值。
            trustee_history_count_before = int(trustee_snapshot.get("trustee_count", 0))
            trustee_trustworthy_count_before = int(trustee_snapshot.get("trustworthy_count", 0))
            trustee_reputation_before = (
                trustee_trustworthy_count_before / trustee_history_count_before
                if trustee_history_count_before > 0
                else None
            )
            ...
            # HR：只缓存本轮记录；整轮结束后统一提交，避免同轮信息泄露。
            self._queue_trustee_reputation_update(
                population_round=population_round,
                interaction_num=interaction_num,
                trustee=trustee,
                sent_amount=trustor_amount,
            ...
            interaction_detail = {
                ...
                "reputation_view_choice": reputation_view_choice,
                "population_round": population_round,
                "trustee_history_count_before_decision": trustee_history_count_before,
                "trustee_trustworthy_count_before_decision": trustee_trustworthy_count_before,
                "trustee_reputation_before_decision": trustee_reputation_before,
            }
            
        except Exception as e:
            logging.error(f"Error in interaction {interaction_num}: {e}")
            if is_population_round_end:
                self._commit_population_round_reputation_updates()
            return {}
...
async def main():
    ...
    EXPERIMENT_CONFIG = {
        ...
        "description": "Trust Game population + HR reputation (view from population round 2; round-start snapshots; trustworthy count/opportunity count; Summary/Detail/Both/None)",
        "game_settings": {
