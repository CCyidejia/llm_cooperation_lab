...
Enhanced with a costly punishment action based on "Winners Don't Punish"
"""
import os
import json
import sys
...
# Keep all four baseline outcomes unchanged; only add outcomes involving Punish.
PAYOFF_MATRIX = {
    ("Yes", "Yes"): (3, 3),
    ("Yes", "No"): (0, 5),
    ("No", "Yes"): (5, 0),
...
# Ensure costly-punishment results directory exists
os.makedirs("result_prisoners_dilemma_group_costly_punishment", exist_ok=True)
...
class PrisonersDilemmaAgent(AgentBase):
    ...
    async def make_decision(self, partner_id: int, partner_name: str, memory_size: int) -> tuple:
        ...
        """
        # History visibility is partner-specific; MEMORY_SIZE controls how many
        # recent interactions with the current partner are shown.
        ...
        user_msg = (
            ...
            "Your decision must be exactly one of **Yes (Cooperate)**, **No (Defect/Betray)**, or **Punish (Costly Punishment)**.\n"
            "Punish is a separate action: if you choose Punish, you cannot also choose Yes or No in this interaction.\n"
            "YOU MUST FOLLOW THIS OUTPUT FORMAT EXACTLY:\n"
            "\n"
            "<output>\n"
            "  <action>Yes, No, or Punish</action>\n"
        ...
        for attempt in range(max_retries):
            try:
                ...
                # --- 解析逻辑 ---
                action_match = re.search(r'<action>\s*(Yes|No|Punish)\s*</action>', content, re.IGNORECASE)
                if action_match:
                    ...
                    match = re.search(r'^\s*(yes|no|punish)\s*[.!]?$', first_line, re.IGNORECASE)
...
class PrisonersDilemmaEnvironment:
    ...
    def __init__(self, num_agents: int, memory_size: int, total_interactions: int, 
        ...
        self.game_stats = {
            ...
            'cooperation_count': 0,
            'cooperative_action_count': 0,
            'defection_action_count': 0,
            'punishment_action_count': 0
        }
        self.game_logs = []
    ...
    async def run_interaction(self, interaction_num: int) -> dict:
        ...
        self.game_stats["cooperative_action_count"] += sum(action == "Yes" for action in actions)
        self.game_stats["defection_action_count"] += sum(action == "No" for action in actions)
        self.game_stats["punishment_action_count"] += sum(action == "Punish" for action in actions)
    ...
    async def save_game_results(self, experiment_name: str = None, experiment_num: int = 1, main_result_dir: str = None):
        ...
        if main_result_dir:
            ...
            result_dir = os.path.join(
                "result_prisoners_dilemma_group_costly_punishment",
                experiment_name
            )
        ...
        with open(stats_path, 'w', encoding='utf-8') as f:
            ...
            total_actions = total_interactions * 2
            self.game_stats['mutual_cooperation_rate'] = (
                self.game_stats['cooperation_count'] / total_interactions
                if total_interactions else 0.0
            )
            self.game_stats['cooperative_action_rate'] = (
                self.game_stats['cooperative_action_count'] / total_actions
                if total_actions else 0.0
            )
            self.game_stats['defection_action_rate'] = (
                self.game_stats['defection_action_count'] / total_actions
                if total_actions else 0.0
            )
            self.game_stats['punishment_action_rate'] = (
                self.game_stats['punishment_action_count'] / total_actions
                if total_actions else 0.0
            )
...
def print_payoff_statistics(env):
    ...
    for interaction in env.game_logs:
        ...
        action_counts[detail['choice1']] += 1
        action_counts[detail['choice2']] += 1
    ...
    total_actions = len(env.game_logs) * 2
    print(f"Cooperative action rate: {action_counts['Yes'] / total_actions * 100:.1f}%")
    print(f"Defection action rate: {action_counts['No'] / total_actions * 100:.1f}%")
    print(f"Punishment action rate: {action_counts['Punish'] / total_actions * 100:.1f}%")
...
async def main():
    ...
    # --- 实验参数设置 ---
    NUM_AGENTS = 24  # 代理数量
    MEMORY_SIZE = 10  # 向Agent展示与当前对手最近10次互动历史
    TOTAL_POPULATION_ROUNDS = 10  # 总群体回合数
    TOTAL_INTERACTIONS = TOTAL_POPULATION_ROUNDS * (NUM_AGENTS // 2)  # 总交互次数
    
    # --- 用户自定义文件夹名称 ---
    base_result_dir = "result_prisoners_dilemma_group_costly_punishment"
    ...
    for experiment_index in range(REPEAT_TIMES):
        ...
        rational_profile = (
            ...
            "In each round, both of you will simultaneously choose exactly one of three actions: "
            "Yes (Cooperate), No (Defect/Betray), or Punish (Costly Punishment). "
            "Punish is a separate action. If you choose Punish, you cannot also choose Yes or No in the same interaction. "
            "The punishment technology has a cost-to-impact ratio of 1:4: it costs the punisher 1 point and imposes a 4-point loss on the punished player. "
            ...
            "- If you choose Yes and [OPPONENT_NAME] chooses Punish: You get -4 points, [OPPONENT_NAME] gets 3 points "
            "- If you choose Punish and [OPPONENT_NAME] chooses Yes: You get 3 points, [OPPONENT_NAME] gets -4 points "
            "- If you choose No and [OPPONENT_NAME] chooses Punish: You get -2 points, [OPPONENT_NAME] gets -1 point "
            "- If you choose Punish and [OPPONENT_NAME] chooses No: You get -1 point, [OPPONENT_NAME] gets -2 points "
            "- If both choose Punish: You get -4 points, [OPPONENT_NAME] gets -4 points "
