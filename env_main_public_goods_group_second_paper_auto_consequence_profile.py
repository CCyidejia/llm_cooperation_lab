...
os.makedirs("result_public_goods_group_reputation_indirect_reciprocity_consequence_prompt", exist_ok=True)
...
class PublicGoodsAgent(AgentBase):
    ...
    async def choose_action(self, initial_endowment: int, public_pool_multiplier: float, num_agents: int, total_rounds: int, memory_size: int = 5):
        ...
        try:
            ...
            match = re.search(r'^\s*(\d+)\b\s*[-–—:,]?\s*(.*)$', content, re.DOTALL)
            if not match:
                match = re.search(r'\b(\d+)\b\s*[-–—:,]?\s*(.*)$', content, re.DOTALL)
            if match:
                ...
                logging.warning(f"[{self.name}] Could not parse contribution from response, using default 0. Response: {content}")
                explanation = "Could not parse contribution; defaulted to 0"
...
async def main(base_experiment_name, experiment_index):
    ...
    base_result_dir = "result_public_goods_group_reputation_indirect_reciprocity_consequence_prompt"
