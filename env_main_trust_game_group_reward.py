...
# -*- coding: utf-8 -*-
"""
Trust Game Population + competitive reward pool (12δ, δ=2) split among trustworthy Trustees.
Based on env_main_trust_game_group.py: same base user prompts and act() role_profile, plus
reward rules in system message and per-round observation broadcast in user message (all English).
"""
import os
import json
...
# Competitive reward: fixed pool 12δ (δ=2); each trustworthy (T) Trustee gets 12δ/(N_T+1)
REWARD_DELTA = 2
POOL_12_DELTA = 12 * REWARD_DELTA

# Appended after act() system role_profile (base profile unchanged)
REWARD_SYSTEM_APPEND = (
    "\n\n[Competitive reward mechanism]\n"
    "Fixed budget: each round the system allocates a total reward budget of 12δ coins (δ=2).\n"
    "Moral classification (applied by the system): if the Trustor sends 0, no trustworthy/untrustworthy "
    "judgment applies to the Trustee; otherwise, if the Trustee returns an amount ≥ 1.5× the amount sent, "
    "the Trustee is trustworthy (T); otherwise untrustworthy (U).\n"
    "Distribution: after each round, count N_T, the number of trustworthy (T) Trustees among the 12 Trustees "
    "in the previous population round; each trustworthy (T) Trustee receives extra coins "
    "Reward = 12δ/(N_T+1). Untrustworthy (U) Trustees and all Trustors receive no such extra reward."
)

class TrustGamePopulationRewardAgent(AgentBase):
    """Agent for Trust Game Population Experiment based on V2 framework"""
    
    def __init__(self, id: int, name: str, profile: str):
        """Initialize TrustGamePopulationRewardAgent"""
        ...
        # Last population round observation text appended to next user prompt (None on first round)
        self._last_reward_broadcast = None
        
        # Setup logger
        self.logger = logging.getLogger(f"TrustGamePopulationRewardAgent.{self._name}")
        
        self.logger.info(f"TrustGamePopulationRewardAgent initialized: {self._name}")
    ...
    async def dump(self) -> dict:
        ...
        try:
            dump_data = {
                "id": self._id,
                "name": self._name,
                "agent_type": "TrustGamePopulationRewardAgent",
                ...
                "last_reward_broadcast": getattr(self, "_last_reward_broadcast", None),
    ...
    async def load(self, dump_data: dict):
        ...
        try:
            ...
            if hasattr(self, "_last_reward_broadcast"):
                self._last_reward_broadcast = dump_data.get("last_reward_broadcast", self._last_reward_broadcast)
    ...
    async def act(self, interaction_num: int, role: str, initial_funds: int,
        ...
        try:
            ...
            if self._last_reward_broadcast:
                prompt = f"{prompt}\n\n{self._last_reward_broadcast}"
            ...
            role_profile = f"{role_profile}{REWARD_SYSTEM_APPEND}"
    ...
    def _parse_amount(self, content: str, role: str, max_amount: int) -> int:
        ...
        # Match only plausible amounts; avoid digits from error messages (e.g. HTTP 402)
        if max_amount <= 10:
    ...
    async def _call_llm_with_retry(self, system_message, user_prompt, max_retries=4):
        ...
        while retries < max_retries:
            try:
                if retries > 0:
                    print(f"[{self.name}] [RETRY] LLM call retry {retries}...")
                ...
                last_exception = e
                print(
                    f"[{self.name}] [ERROR] LLM call failed (attempt {retries + 1}/{max_retries}): "
...
def apply_population_round_reward_and_broadcast(
    agents: list,
    env: TrustGamePopulationEnvironment,
    reward_delta: int = REWARD_DELTA,
) -> dict:
    """
    After each population round (12 pairings): moral classification, credit reward payouts,
    and set each agent's next user-prompt last-round observation block.
    """
    pairs_per_round = env.num_agents // 2
    last_logs = env.game_logs[-pairs_per_round:]

    pool_12delta = 12 * reward_delta
    moral_rows = []
    for log in last_logs:
        d = log.get("detail") or {}
        sent = int(d.get("sent_amount", 0) or 0)
        returned = int(d.get("returned_amount", 0) or 0)
        if sent == 0:
            moral = "exempt"
            is_T = False
        else:
            is_T = returned >= 1.5 * sent
            moral = "T" if is_T else "U"
        moral_rows.append({
            "trustor_id": d.get("trustor_id"),
            "trustee_id": d.get("trustee_id"),
            "sent": sent,
            "returned": returned,
            "moral": moral,
            "is_T": is_T,
    ...
    N_T = sum(1 for r in moral_rows if r["is_T"])
    reward_per_T = (pool_12delta / (N_T + 1)) if N_T > 0 else 0.0

    agent_by_id = {a._id: a for a in agents}
    extras_by_trustee_id = {}

    for log, r in zip(last_logs, moral_rows):
        d = log.setdefault("detail", {})
        d["trustee_moral_status"] = r["moral"]
        d["trustee_is_trustworthy_T"] = r["is_T"]
        d["reward_extra_trustee"] = round(reward_per_T, 6) if r["is_T"] else 0.0

    for r in moral_rows:
        if r["is_T"]:
            tid = r["trustee_id"]
            extras_by_trustee_id[tid] = reward_per_T
            agent_by_id[tid]._total_payoff += reward_per_T

    header = (
        f"[Last round observation] Among the 12 Trustees in the last population round, "
        f"{N_T} were trustworthy (T). "
        f"The system's fixed reward pool for this round is {pool_12delta} coins (12δ, δ={reward_delta})."
    )
    ...
    for agent in agents:
        ...
        for r in moral_rows:
            if r["trustor_id"] == aid:
                msg = (
                    f"{header} You acted as Trustor last round and do not receive any extra reward from the pool."
                )
                break
            if r["trustee_id"] == aid:
                if r["moral"] == "exempt":
                    msg = (
                        f"{header} You acted as Trustee last round; because the Trustor sent 0, "
                        "trustworthiness was not judged and you did not participate in the pool split."
                    )
                elif r["is_T"]:
                    msg = (
                        f"{header} You acted as Trustee last round and were classified as trustworthy (T); "
                        f"you received {reward_per_T:.2f} extra coins from the pool."
                    )
                else:
                    msg = (
                        f"{header} You acted as Trustee last round and were classified as untrustworthy (U); "
                        "you received no extra reward."
                    )
                break
        agent._last_reward_broadcast = (
            msg
            if msg
            else f"{header} (Could not match your role in internal records for the last round.)"
        )

    return {"N_T": N_T, "reward_per_T": reward_per_T, "extras_by_trustee_id": extras_by_trustee_id}


class TrustGamePopulationRewardEnvironment(TrustGamePopulationEnvironment):
    """Clears each agent's last-round broadcast on reset to avoid carryover across runs."""

    def reset(self):
        super().reset()
        for agent in self.agents:
            if hasattr(agent, "_last_reward_broadcast"):
                agent._last_reward_broadcast = None
...
async def main():
    ...
    EXPERIMENT_CONFIG = {
        "experiment_name": "TrustGame_Population_Experiment_CompetitiveReward",
        "description": "Trust Game population (24 agents) + competitive reward pool 12δ (δ=2) for trustworthy Trustees",
        "game_settings": {
    ...
    try:
        ...
        base_result_dir = "result_trust_game_population_reward"
        ...
        NUM_GAMES = 1  # independent full runs per script execution
        ROUNDS_PER_GAME = 30
        ...
        for game_num in range(NUM_GAMES):
            ...
            env = TrustGamePopulationRewardEnvironment(
                num_agents=EXPERIMENT_CONFIG["game_settings"]["num_agents"],
                ...
                try:
                    ...
                    if interaction_result:
                        ...
                        if interaction_num % (num_agents // 2) == 0:
                            ...
                            ret_rw = apply_population_round_reward_and_broadcast(
                                agents, env, REWARD_DELTA
                            )
                            print(
                                f"  [Competitive reward] Trustworthy Trustees N_T={ret_rw['N_T']}, "
                                f"extra Reward per T={ret_rw['reward_per_T']:.2f}"
                            )
                            for tid, amt in ret_rw["extras_by_trustee_id"].items():
                                tname = next(a.name for a in agents if a._id == tid)
                                experiment_stats["overall_metrics"]["total_payoffs_per_agent"][tname] += amt
                                game_stats["agent_payoffs"][tname] += amt
...
async def _create_agents(config: dict, llm_agent: AgentLLM) -> list:
    ...
    for i in range(num_agents):
        ...
        agent = TrustGamePopulationRewardAgent(
            id=i,
            name=agent_name,
