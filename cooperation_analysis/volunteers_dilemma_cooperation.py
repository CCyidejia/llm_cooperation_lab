...
该脚本用于计算志愿者博弈群体实验的合作率，
支持分析多个实验文件，计算每轮的合作率以及特定轮次的平均合作率。

与非群体实验计算规则一致：
- 合作率 = 志愿者数量 / 玩家数量
- 每个game_logs文件代表一次完整的实验
- 实验包含多轮（如30轮），每轮包含所有agent的选择
- 总体合作率 = 各实验合作率的平均（默认只分析 1 个 game_logs 文件时即为该次实验合作率）
"""
...
data_dir = r"d:\常用\agentsociety\agentsociety\packages\agentsociety2\result_volunteer_dilemma_group\052101_gpt-5.5\data"

# 定义实验配置
NUM_ROUNDS_PER_GAME = 30  # 每个实验的轮数
NUM_AGENTS = 24  # 每轮的agent数量
# 只分析前 N 个 game_logs*.json（与 env 仅运行 1 次实验时通常只有 game_logs1.json 一致）
MAX_GAME_LOG_FILES = 3

# 尝试获取数据目录中的所有game_logs文件
try:
    file_names = [f for f in os.listdir(data_dir) if f.startswith('game_logs') and f.endswith('.json')]
    file_names = sorted(file_names)[:MAX_GAME_LOG_FILES]  # 排序以确保顺序一致，并限制数量
    if not file_names:
...
for file_name in file_names:
    ...
    try:
        ...
        for round_idx, round_data in enumerate(game_logs):
            ...
            # 获取该轮的志愿者数量
            if 'num_volunteers' in round_data:
                num_volunteers = round_data['num_volunteers']
                
                # 计算该轮的合作率：志愿者数量 / agent总数
                round_cooperation_rate = num_volunteers / NUM_AGENTS
                round_cooperation_rates[round_num].append(round_cooperation_rate)
                experiment_cooperation_scores.append(round_cooperation_rate)
