...
os.makedirs("result_public_goods_group_positive_interactions", exist_ok=True)
...
# Limit simultaneous LLM requests to avoid overloading the API gateway.
LLM_CONCURRENCY = 6
...
class PublicGoodsAgent(AgentBase):
    ...
    def __init__(self, id: int, name: str, llm_semaphore: asyncio.Semaphore, profile: str = ""):
    ...
    async def choose_action(self, initial_endowment: int, public_pool_multiplier: float, num_agents: int, total_rounds: int, memory_size: int = 5):
        ...
        try:
            ...
            if match:
                ...
                if keyword_match:
                    ...
                    if self.min_contribution <= parsed_contribution <= self.max_contribution:
                        ...
                        contribution = 0
                        explanation = "Parsed contribution out of range; defaulted to 0."
                else:
                    contribution = 0
            ...
            fallback = self._get_stable_choice()
            self._previous_choices.append(fallback)
            return fallback, f"LLM call failed; used stable fallback {fallback}: {str(e)}"
    ...
    async def _call_llm_with_retry(self, system_message: str, user_prompt: str, max_retries: int = 5, retry_delay: int = 2) -> str:
        for attempt in range(max_retries):
            try:
                ...
                if generated_text.strip().startswith("API Call Failed:"):
                ...
                if attempt < max_retries - 1:
                    ...
                    logging.info(f"[{self.name}] Retrying LLM call in {backoff} seconds")
...
async def main(base_experiment_name, experiment_index):
    ...
    base_result_dir = "result_public_goods_group_positive_interactions"
    ...
    # asyncio synchronization primitives must not be shared across the
    # separate event loops created for repeated experiment runs.
