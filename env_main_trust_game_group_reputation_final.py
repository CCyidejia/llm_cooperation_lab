...
class TrustGamePopulationReputationAgent(AgentBase):
    ...
    async def act(self, interaction_num: int, role: str, initial_funds: int,
        ...
        try:
            ...
            # 论文的 Baseline/HR 不自动向 Trustor 展示双方的私人交互历史。
            # Trustee 路径保持原文件行为不变。
            history_str = (
                "No private bilateral interaction history is provided."
                if role == "Trustor"
                else self._build_partner_history_string(partner_name)
            )
    ...
    async def act_trustor_investment(
        ...
        try:
            self._current_funds = current_funds
            history_str = "No private bilateral interaction history is provided."
